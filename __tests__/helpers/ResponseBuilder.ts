...
export class ResponseBuilder {
   ...
   */
  constructor() {
    this.response = helpers.partial<ResponseEnvelope>({
  ...
  public addSessionAttributes(attribute: { [key: string]: any }): ResponseBuilder {
    switch (true) {
      case _.isNil(this.response.sessionAttributes):
        this.response.sessionAttributes = attribute;
        break;
      default:
        this.response.sessionAttributes = _.merge(this.response.sessionAttributes, attribute);
        break;
    }
    return this;
  }
  ...
  public addResponse(key: string, ...args: any[]): ResponseBuilder {
    ...
    this.response.response.outputSpeech = helpers.partial<ui.OutputSpeech>({
  ...
  public addReprompt(key: string, ...args: any[]): ResponseBuilder {
    ...
    this.response.response.reprompt = helpers.partial<ui.Reprompt>({
      outputSpeech: {
        ssml: expect.stringMatching(response),
      },
    });
  ...
  public addSimpleCard(title: { key: string; args?: any[] }, content: { key: string; args?: any[] }): ResponseBuilder {
    this.response.response.card = helpers.partial<ui.Card>({
      type: 'Simple',
