...
export class IntentRequestBuilder extends BaseRequestBuilder<IntentRequestBuilder> {
  ...
  public addIntent(intent: { name: string; confirmationStatus?: IntentConfirmationStatus }): IntentRequestBuilder {
  ...
  public addSlots(slot: {
    ...
    _.mapKeys(slot, (val, key) => {
      this.getRequestByType<IntentRequest>().intent.slots[key] = helpers.partial<Slot>({
        ...
        value: val.value,
        resolutions:
          _.isNil(val.resolutions) || _.isEmpty(val.resolutions)
            ? undefined
            : helpers.partial<slu.entityresolution.Resolutions>({
                resolutionsPerAuthority: _.map(
                  val.resolutions,
                  (res) =>
                    // eslint-disable-next-line implicit-arrow-linebreak
                    helpers.partial<slu.entityresolution.Resolution>({
                      status: {
                        code: res.status,
                      },
